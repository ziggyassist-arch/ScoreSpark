...
function FormationRow({ players, ratings, events, view = "ratings" }: {
  players: { number: number; name: string; headshot?: string; id?: string; age?: number; nationality?: string }[];
        ...
        return (
          <div key={p.number} className="flex flex-col items-center w-12 sm:w-16">
            <div className="relative">
              ...
              {view === "age" && p.age && (
                <div className="absolute -bottom-1 -right-1 w-[18px] h-[18px] sm:w-5 sm:h-5 rounded-full bg-blue-500/80 flex items-center justify-center shadow-sm">
                  <span className="text-[7px] sm:text-[8px] font-bold text-white tabular-nums">{p.age}</span>
                </div>
              )}
              {view === "country" && p.nationality && (
                <div className="absolute -bottom-1 -right-1 px-1 h-[16px] sm:h-[18px] rounded-full bg-white/20 flex items-center justify-center shadow-sm">
                  <span className="text-[7px] sm:text-[8px] font-medium text-white">{p.nationality.slice(0, 3).toUpperCase()}</span>
                </div>
              )}
              {/* Event badges */}
...
type FormationView = "ratings" | "age" | "country" | "number";

function FullPitchFormation({ lineups, match }: { lineups: { home: Lineup; away: Lineup }; match: Match }) {
  const [view, setView] = useState<FormationView>("ratings");
  const views: { key: FormationView; label: string }[] = [
    { key: "ratings", label: "Season stats" },
    { key: "number", label: "Jersey №" },
    { key: "age", label: "Age" },
    { key: "country", label: "Country" },
  ];

  function buildRows(lineup: Lineup): { number: number; name: string; headshot?: string; id?: string; age?: number; nationality?: string }[][] {
  ...
  return (
    <div className="space-y-4">
      {/* Formation view toggle — FotMob style */}
      <div className="flex items-center justify-center gap-1 bg-white/[0.03] rounded-lg p-1">
        {views.map(v => (
          <button
            key={v.key}
      ...
      <div className="relative pitch-bg rounded-2xl overflow-hidden max-w-[850px] mx-auto">
        ...
        <div className="relative z-10 px-4 pt-6 pb-3">
          ...
          <div className="flex flex-col gap-4 sm:gap-5">
            {homeRows.map((row, i) => (
              <FormationRow key={i} players={row} ratings={view === "ratings" ? homeRatings : undefined} events={match.events.filter(e => e.team === "home")} view={view} />
            ))}
          </div>
        </div>
        ...
        <div className="relative z-10 px-4 pt-3 pb-6">
          <div className="flex flex-col-reverse gap-4 sm:gap-5">
            {awayRows.map((row, i) => (
              <FormationRow key={i} players={row} ratings={view === "ratings" ? awayRatings : undefined} events={match.events.filter(e => e.team === "away")} view={view} />
            ))}
          </div>
